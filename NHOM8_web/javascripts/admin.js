...
function layThongTinSanPhamTuTable(id) {
    ...
    var company = tr[3].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
    var img = khung.querySelector('.hinhDaiDien').src;
    var price = khung.querySelector('.inputPrice').value;


    var chatlieu =khung.querySelector('.inputChatLieu').value;
    var kichthuoc =khung.querySelector('.inputSize').value;
    var khungvien =khung.querySelector('.inputBorderColor').value;
    var mau =khung.querySelector('.inputColor').value;
    var khac =khung.querySelector('.inputMore').value;
    var chieudai =khung.querySelector('.inputHeight').value;
    var chieurong = khung.querySelector('.inputWidth').value;
    if (isNaN(price)) {
...
function addKhungSuaSanPham(masp) {
    ...
    <table class="overlayTable table-outline table-content table-header">
        ...
        <tr>
            ...
            <td><input class="inputPrice" type="text" value="`+ stringToNum(sp.price) + `"></td>
        </tr>
        <tr>
        ...
        <tr>
            <td>Chất liệu :</td>
            <td><input class="inputChatLieu" type="text" value="`+ sp.detail.chatlieu + `"></td>
        </tr>
        <tr>
            <td>Kích thước :</td>
            <td><input class="inputSize" type="text" value="`+ sp.detail.size + `"></td>
        </tr>
        <tr>
            <td>Khung viền :</td>
            <td><input class="inputBorderColor" type="text" value="`+ sp.detail.border_color+ `"></td>
        </tr>
        <tr>
            <td>Màu :</td>
            <td><input  class="inputColor" type="text" value="`+ sp.detail.color + `"></td>
        </tr>
        <tr>
            <td>Khác :</td>
            <td><input  class="inputMore" type="text" value="`+ sp.detail.more + `"></td>
        </tr>
        <tr>
            <td>Chiều dài :</td>
            <td><input  class="inputHeight" type="text" value="`+ sp.detail.height + `"></td>
        </tr>
        <tr>
            <td>Chiều rộng :</td>
            <td><input  class="inputWidth" type="text" value="`+ sp.detail.width + `"></td>
        </tr>
    ...
    for (var i = 0; i < u.length; i++) {
        for (var j = 0; j < u[i].donhang.length; j++) {
            ...
            for (var s of u[i].donhang[j].sp) {
                console.log(s.masp)
                var timsp = timKiemTheoMa(list_products, s.masp);
                console.log(timsp);
            ...
            var sps = '';
            for (var s of u[i].donhang[j].sp) {
                sps += `<p style="text-align: right">` + (timKiemTheoMa(list_products, s.masp).name + ' [' + s.soluong + ']') + `</p>`;
            }
            ...
            for (var s of u[i].donhang[j].sp) {
                danhSachSanPham.push({
                    sanPham: timKiemTheoMa(list_products, s.masp),
                    soLuong: s.soluong,
                });
            }
...
function getValueOfTypeInTable_KhachHang(tr, loai) {
    ...
    switch (loai) {
        ...
        case 'taikhoan': return td[2].innerHTML.toLowerCase();
        case 'matkhau': return td[3].innerHTML.toLowerCase();
    }
    return false;
}
